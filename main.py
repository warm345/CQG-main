...
from mindspore.optim import Adam
...
def set_seed(args):
    ...
    mindspore.manual_seed(args.seed)
    if 'cuda' in args.device:
        mindspore.cuda.manual_seed_all(args.seed)
...
def train(config: Config, train_dataloader: DataLoader, num_epochs: int, early_stop: int, 
          bert_model_name: str, dev: mindspore.device, valid_dataloader: DataLoader = None, tokenizer=None):
    metric = None
    ...
    if config.fp16:
        scaler = mindspore.cuda.amp.GradScaler(enabled=bool(config.fp16))
    ...
    for epoch in range(num_epochs):
        ...
        for iter, batch in tqdm(enumerate(train_dataloader, 1), desc="--training batch", total=len(train_dataloader)):
            ...
            with mindspore.cuda.amp.autocast(enabled=bool(config.fp16)):
                outputs = model(input_ids, decoder_input_ids, flag)
            loss = outputs['loss']
            loss = mindspore.sum(loss)
            if config.fp16:
        ...
        if valid_dataloader is not None:
            ...
            if accuracy > best_accuracy:
                ...
                stop_num = 0
                mindspore.save(model.state_dict(), 'model.pth')
    ...
    modeldict = mindspore.load('model.pth')
    model.load_state_dict(modeldict)
    return model


def test(config: Config, valid_dataloader: DataLoader, model: nn.Module, dev: mindspore.device,
         tokenizer):
    model.eval()
    predictions_text = []
    targets_text = []
    predictions = []
    targets = []
    with mindspore.no_grad(), mindspore.cuda.amp.autocast(enabled=False):
...
def main():
    ...
    if opt.mode == "train":
        ...
        train_dataloader = ds.GeneratorDataset(dataset, shuffle=True, num_parallel_workers=conf.num_workers)
        print("[Data Info] Loading validation data", flush=True)
        valid_dataloader = ds.GeneratorDataset(eval_dataset, shuffle=True, num_parallel_workers=conf.num_workers)
        print("[Data Info] Loading test data", flush=True)
        test_dataloader = ds.GeneratorDataset(test_dataset, shuffle=True, num_parallel_workers=conf.num_workers)
        ...
        if conf.gpus:
            device_ids = [0, 1, 2, 3]
            model = nn.DataParallel(model, device_ids=device_ids, output_device=0)
        modeldict = mindspore.load(f'model.pth')
        model.load_state_dict(modeldict)
