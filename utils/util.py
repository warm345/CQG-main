...
def get_optimizers(config: Config, model: nn.Module, num_training_steps: int, weight_decay:float = 0.01,
                   warmup_step: int = -1, eps:float = 1e-8) -> Tuple[mindspore.optim.Optimizer, mindspore.optim.lr_scheduler.LambdaLR]:
...
def clip_gradients(model, gradient_clip_val, device):
    # this code is a modification of mindspore.nn.utils.clip_grad_norm_
    # with TPU support based on https://github.com/mindspore/xla/blob/master/TROUBLESHOOTING.md
    if gradient_clip_val > 0:
        ...
        if norm_type == math.inf:
            ...
            total_norm = mindspore.zeros([], device=device if parameters else None)
            for p in parameters:
                param_norm = p.grad.data.pow(norm_type).sum()
                total_norm.add_(param_norm)
            total_norm = (total_norm ** (1. / norm_type))
        eps = EPSILON
        clip_coef = mindspore.tensor(max_norm, device=device) / (total_norm + eps)
        for p in parameters:
            p.grad.data.mul_(mindspore.where(clip_coef < 1, clip_coef, mindspore.tensor(1., device=device)))
...
def extract_module(model_file: str = "model_files/mtl/model.m"):
    ...
    model.load_state_dict(mindspore.load(model_file, map_location=dev))
    mindspore.save(model.model.state_dict(), f"model_files/mtl/mtl_model.m")
