...
class Seq2SeqDecoder(nn.Module):
    ...
    def forward(self, tokens, state, flag, **kwargs):
        """
        :param mindspore.LongTensor tokens: bsz x max_len
    ...
    def reorder_states(self, indices, states):
        ...
        :param mindspore.LongTensor indices:
        :param State states:
        :return:
        """
    ...
    def init_state(self, encoder_output, encoder_mask):
        ...
        :param Union[mindspore.Tensor, list, tuple] encoder_output: 如果不为None，内部元素需要为mindspore.Tensor, 默认其中第一维是batch
            维度
        :param Union[mindspore.Tensor, list, tuple] encoder_mask: 如果部位None，内部元素需要mindspore.Tensor, 默认其中第一维是batch
            维度
        :param kwargs:
    ...
    def decode(self, tokens, state, flag):
        """
        根据states中的内容，以及tokens中的内容进行之后的生成。
        :param mindspore.LongTensor tokens: bsz x max_len, 截止到上一个时刻所有的token输出。
        :param State state: 记录了encoder输出与decoder过去状态
        :return: mindspore.FloatTensor: bsz x vocab_size, 输出的是下一个时刻的分布
        """
        outputs = self(state=state, tokens=tokens, flag=flag)
        if isinstance(outputs, mindspore.Tensor):
            return outputs[:, -1]
        else:
...
class TiedEmbedding(nn.Module):
    ...
    def forward(self, x):
        """
        :param mindspore.FloatTensor x: bsz x * x embed_size
        :return: mindspore.FloatTensor bsz x * x vocab_size
        """
        return mindspore.matmul(x, self.weight.t())
...
class LSTMSeq2SeqDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tokens, state, return_attention=False):
        """
        :param mindspore.LongTensor tokens: batch x max_len
        ...
        decoder_out = mindspore.stack(decoder_out, dim=1)  # batch,seq_len,hidden
        decoder_out = self.dropout_layer(decoder_out)
        if attn_weights is not None:
            attn_weights = mindspore.cat(attn_weights, dim=1)  # batch, tgt_len, src_len
    ...
    def init_state(self, encoder_output, encoder_mask) -> LSTMState:
        ...
        :param mindspore.ByteTensor encoder_mask: bsz x max_len, 为0的位置是padding, 用来指示source中哪些不需要attend
        :return:
        """
        if not isinstance(encoder_output, mindspore.Tensor):
...
class TransformerSeq2SeqDecoderLayer(nn.Module):
    def __init__(self, d_model = 512, n_head = 8, dim_ff = 2048, dropout = 0.1, layer_idx = None):
        ...
        self.encoder_attn = FLAGMultiHeadAttention(d_model, n_head, dropout, layer_idx)
    ...
    def forward(self, x, encoder_output, flag, encoder_mask=None, self_attn_mask=None, state=None):
        """
...
class TransformerSeq2SeqDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tokens, state, flag, return_attention=False):
        """
        :param mindspore.LongTensor tokens: batch x tgt_len，decode的词
        ...
        if self.pos_embed is not None:
            position = mindspore.arange(state.decode_length, state.decode_length+tokens.size(1)).long().to(device)[None]
    ...
    def init_state(self, encoder_output, encoder_mask):
        ...
        :param mindspore.FloatTensor encoder_output: bsz x max_len x d_model, encoder的输出
        :param mindspore.ByteTensor encoder_mask: bsz x max_len, 为1的位置需要attend。
        :return: TransformerState
        """
        if isinstance(encoder_output, mindspore.Tensor):
            encoder_output = encoder_output
    ...
    def _get_triangle_mask(tokens):
        ...
        return mindspore.tril(tensor).byte()

class FLAGMultiHeadAttention(nn.Module):
    """ 
    Attention is all you need中提到的多头注意力
    """
    def __init__(self, d_model: int = 512, n_head: int = 8, dropout: float = 0.0, layer_idx: int = None):
        super(FLAGMultiHeadAttention, self).__init__()
        self.d_model = d_model
        self.n_head = n_head
        self.dropout = dropout
        self.head_dim = d_model // n_head
        self.layer_idx = layer_idx
        self.k_flag = nn.Embedding(3, d_model)
        self.v_flag = nn.Embedding(3, d_model)
    ...
    def forward(self, query, key, value, flag, key_mask=None, attn_mask=None, state=None):
        """
        ...
        if prev_k is not None:
            k = mindspore.cat((prev_k, k), dim=1)
            v = mindspore.cat((prev_v, v), dim=1)
        ...
        rel_k = self.k_flag(flag) #batch x lenk x lenq x dim
        #print(rel_k.size())
        #print(batch_size, k_len, q_len)
        rel_k = rel_k.reshape(batch_size, k_len, q_len, self.n_head, self.head_dim) #b k q n h
        ...
        rel_k = mindspore.matmul(rel_k.permute(1,0,2,3,4), q.transpose(2,3)) # kbqnn
        rel_k = mindspore.mean(rel_k, dim=-1) #kbqn
        rel_k = rel_k.permute(1,2,0,3)
        score = mindspore.einsum('bqnh,bknh->bqkn', q, k)  # bs,q_len,k_len,n_head
        #flag_score = mindspore.einsum('bqnh,bknh->bqkn', q, rel_k)  # bs,q_len,k_len,n_head
        attn_weights = rel_k + score
        ...
        output = mindspore.einsum('bqkn,bknh->bqnh', attn_weights, v)  # batch,q_len,n_head,head_dim
