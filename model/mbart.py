...
def shift_tokens_right(input_ids: mindspore.Tensor, pad_token_id: int):
    """
...
def _make_causal_mask(input_ids_shape: mindspore.Size, dtype: mindspore.dtype, past_key_values_length: int = 0):
    ...
    mask = mindspore.full((tgt_len, tgt_len), float("-inf"))
    mask_cond = mindspore.arange(mask.size(-1))
    mask.masked_fill_(mask_cond < (mask_cond + 1).view(mask.size(-1), 1), 0)
    mask = mask.to(dtype)

    if past_key_values_length > 0:
        mask = mindspore.cat([mindspore.zeros(tgt_len, past_key_values_length, dtype=dtype), mask], dim=-1)
...
def _expand_mask(mask: mindspore.Tensor, dtype: mindspore.dtype, tgt_len: Optional[int] = None):
    ...
    return inverted_mask.masked_fill(inverted_mask.bool(), mindspore.finfo(dtype).min)
...
class MBartLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids_shape: mindspore.Size, past_key_values_length: int = 0):
        """`input_ids_shape` is expected to be [bsz x seqlen]."""
        bsz, seq_len = input_ids_shape[:2]
        positions = mindspore.arange(
            past_key_values_length, past_key_values_length + seq_len, dtype=mindspore.long, device=self.weight.device
        )
...
class MBartAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_flag = nn.Embedding(3, self.embed_dim)
        self.v_flag = nn.Embedding(3, self.embed_dim)
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if flag != None:
                ...
                #rel_k = mindspore.mean(rel_k, dim=1)
                rel_v = self.v_flag(flag)
                #rel_v = mindspore.mean(rel_v, dim=1)
                rel_k = self._shape(self.k_proj(key_value_states), -1, bsz)
                rel_v = self._shape(self.k_proj(key_value_states), -1, bsz)
            key_states = self._shape(self.k_proj(key_value_states), -1, bsz)
            value_states = self._shape(self.v_proj(key_value_states), -1, bsz) # batch, head_num q_len, head_dim
            
        elif past_key_value is not None:
            # reuse k, v, self_attention
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
            key_states = mindspore.cat([past_key_value[0], key_states], dim=2)
            value_states = mindspore.cat([past_key_value[1], value_states], dim=2)
        else:
            # self_attention
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
        ...
        if rel_k != None:
            rel_k = rel_k.view(*proj_shape)
            rel_v = rel_v.view(*proj_shape)
        value_states = value_states.view(*proj_shape)

        src_len = key_states.size(1)
        if rel_k != None:
            key_states = key_states + rel_k
            value_states = value_states + rel_v
        attn_weights = mindspore.bmm(query_states, key_states.transpose(1, 2))
        #if rel_k != None:
            #attn_weights += mindspore.bmm(query_states, rel_k.transpose(1, 2))
...
class MBartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            hidden_states (`mindspore.FloatTensor`): input to the layer of shape *(seq_len, batch, embed_dim)*
            attention_mask (`mindspore.FloatTensor`): attention mask of size
                *(batch, 1, tgt_len, src_len)* where padding elements are indicated by very large negative values.
            layer_head_mask (`mindspore.FloatTensor`): mask for attention heads in a given layer of size
        ...
        if hidden_states.dtype == mindspore.float16 and (
            mindspore.isinf(hidden_states).any() or mindspore.isnan(hidden_states).any()
        ):
            clamp_value = mindspore.finfo(hidden_states.dtype).max - 1000
            hidden_states = mindspore.clamp(hidden_states, min=-clamp_value, max=clamp_value)
...
class MBartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`mindspore.FloatTensor`): attention mask of size
                *(batch, 1, tgt_len, src_len)* where padding elements are indicated by very large negative values.
            encoder_hidden_states (`mindspore.FloatTensor`):
                cross attention input to the layer of shape *(seq_len, batch, embed_dim)*
            encoder_attention_mask (`mindspore.FloatTensor`): encoder attention mask of size
                *(batch, 1, tgt_len, src_len)* where padding elements are indicated by very large negative values.
            layer_head_mask (`mindspore.FloatTensor`): mask for attention heads in a given layer of size
                *(encoder_attention_heads,)*.
            cross_attn_layer_head_mask (`mindspore.FloatTensor`): mask for cross-attention heads in a given layer of
                size *(decoder_attention_heads,)*.
            past_key_value (`Tuple(mindspore.FloatTensor)`): cached past key and value projection states
            output_attentions (`bool`, *optional*):
                Whether or not to return the attentions tensors of all attention layers. See `attentions` under
                returned tensors for more detail.
        """
...
class MBartClassificationHead(nn.Module):
    ...
    def forward(self, hidden_states: mindspore.Tensor):
        ...
        hidden_states = mindspore.tanh(hidden_states)
        hidden_states = self.dropout(hidden_states)
...
MBART_START_DOCSTRING = r"""
    ...
    etc.)
    This model is also a mindspore [mindspore.nn.Module](https://mindspore.org/docs/stable/nn.html#mindspore.nn.Module) subclass.
...
MBART_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_input_ids (`mindspore.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
        ...
        decoder_attention_mask (`mindspore.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
        ...
        cross_attn_head_mask (`mindspore.Tensor` of shape `(decoder_layers, decoder_attention_heads)`, *optional*):
            ...
            - 1 indicates the head is **not masked**,
            - 0 indicates the head is **masked**.
        encoder_outputs (`tuple(tuple(mindspore.FloatTensor)`, *optional*):
        ...
        past_key_values (`tuple(tuple(mindspore.FloatTensor))`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            Tuple of `tuple(mindspore.FloatTensor)` of length `config.n_layers`, with each tuple having 2 tensors of shape
            ...
            ``decoder_input_ids``` of shape `(batch_size, sequence_length)`. inputs_embeds (`mindspore.FloatTensor` of
            ...
            matrix.
        decoder_inputs_embeds (`mindspore.FloatTensor` of shape `(batch_size, target_sequence_length, hidden_size)`, *optional*):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                - 0 for tokens that are **masked**.
                [What are attention masks?](../glossary#attention-mask)
            encoder_hidden_states (`mindspore.FloatTensor` of shape `(batch_size, encoder_sequence_length, hidden_size)`, *optional*):
                Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention
                of the decoder.
            encoder_attention_mask (`mindspore.LongTensor` of shape `(batch_size, encoder_sequence_length)`, *optional*):
            ...
            past_key_values (`tuple(tuple(mindspore.FloatTensor))`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
                ...
                all ``decoder_input_ids``` of shape `(batch_size, sequence_length)`. inputs_embeds (`mindspore.FloatTensor`
...
class MBartForConditionalGeneration(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig):
        ...
        self.register_buffer("final_logits_bias", mindspore.zeros((1, self.model.shared.num_embeddings)))
    ...
    def _resize_final_logits_bias(self, new_num_tokens: int) -> None:
        ...
        if new_num_tokens <= old_num_tokens:
            ...
            extra_bias = mindspore.zeros((1, new_num_tokens - old_num_tokens), device=self.final_logits_bias.device)
            new_bias = mindspore.cat([self.final_logits_bias, extra_bias], dim=1)
...
class MBartForSequenceClassification(MBartPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        labels (`mindspore.LongTensor` of shape `(batch_size,)`, *optional*):
        ...
        if len(mindspore.unique_consecutive(eos_mask.sum(1))) > 1:
        ...
        if labels is not None:
            if self.config.problem_type is None:
                ...
                elif self.config.num_labels > 1 and (labels.dtype == mindspore.long or labels.dtype == mindspore.int):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                - 0 for tokens that are **masked**.
                [What are attention masks?](../glossary#attention-mask)
            encoder_hidden_states  (`mindspore.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
                Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention
                if the model is configured as a decoder.
            encoder_attention_mask (`mindspore.FloatTensor` of shape `(batch_size, sequence_length)`, *optional*):
